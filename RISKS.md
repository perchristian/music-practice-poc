...
| Chord detection is wrong or misleading when harmonic information is weak | Medium | Medium | Display cues as approximate; estimate downbeat before placing chord cues; use full-mix harmony plus bass/root evidence; prefer bar-aligned conservative labels over weakly supported extensions | Partially mitigated. Phase 2H adds first-pass downbeat/bar-aligned chroma analysis verified on generated C/Am/F/G test media with pre-roll and local calibration job `f9e9cf9d`, which now returns 106 BPM, 4/4, and C, Dm, Am, Am, C, Dm, Am, Am. More varied real screen recordings still need manual inspection |
