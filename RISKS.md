...
| Chord detection is wrong or misleading when harmonic information is weak | Medium | Medium | Display cues as approximate; preserve user-owned charts; benchmark reference-timed and end-to-end analysis separately on a fixed RWC-P development/holdout split; use Tiny AAM/GuitarSet only to localize mix or timbre failures; compare an external analyzer before repeated local tuning | Partially mitigated. Generated known-answer fixtures cover narrow regressions, and corrected-timing analysis works technically, but the 279-cue `TeAmo.mov` result has not established musical accuracy. Dataset research selected RWC-P v2 as the primary objective benchmark; the harness and baseline remain to be implemented |
| Benchmark results overfit one corpus or fail to represent screen-recording audio | Medium | High | Stratify RWC-P by chord and instrumentation complexity; keep an untouched holdout; use synthetic/solo-instrument corpora only for diagnostics; finish with deterministic AAC/MOV transcodes and 2-3 real iOS screen recordings | Open. The benchmark strategy is documented, but no dataset baseline has run yet; RWC-P is CC BY-NC and must stay outside shipped/demo media |
