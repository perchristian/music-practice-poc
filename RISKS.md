...
| Beat/bar grid is wrong, making chord placement, loops, count-in, and practice notes misleading | Medium | High | Make the grid audible with a metronome/click; let users correct bar 1, tempo, time signature, and offset; store user corrections separately from analyzer output | Partially mitigated. Phase 3A renders beat/bar markers and provides a grid-aligned metronome with persisted click settings. Phase 3B persists manual BPM, bar 1/downbeat start, 4/4 vs 3/4, and small offset corrections in `practiceState.gridOverrides`; real-song manual alignment checks remain open, and a waveform view may be needed if 0.01-second nudging is not intuitive enough |
