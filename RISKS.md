...
| Chord detection is wrong or misleading when harmonic information is weak | Medium | Medium | Display cues as approximate; preserve user-owned charts; benchmark reference-timed and end-to-end analysis separately on a fixed RWC-P development/holdout split; use Tiny AAM/GuitarSet only to localize mix or timbre failures; compare an external analyzer before repeated local tuning | Confirmed as a material limitation. Development MajMin is 58.6% with oracle timing and 53.7% end-to-end; root errors exceed quality errors, and no complexity stratum is reliably strong. The harness makes changes measurable, but user-facing accuracy is not validated |
| Benchmark results overfit one corpus or fail to represent screen-recording audio | Medium | High | Stratify RWC-P by chord and instrumentation complexity; keep an untouched holdout; use synthetic/solo-instrument corpora only for diagnostics; finish with deterministic AAC/MOV transcodes and 2-3 real iOS screen recordings | Partially mitigated by a locked low/medium/high 8-development/4-holdout pilot and protected split defaults. RWC-P remains cleaner than screen-recording audio and is CC BY-NC, so media stays outside shipped/demo assets |
