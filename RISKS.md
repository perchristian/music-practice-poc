...
| Chord detection is wrong or misleading when harmonic information is weak | Medium | Medium | Display cues as approximate; preserve user-owned charts; benchmark reference-timed and end-to-end analysis separately on a fixed RWC-P development/holdout split; retain suppressed candidates separately so users can recover plausible short changes explicitly; compare an external analyzer before repeated local tuning | Confirmed as a material limitation. Isolated-beat smoothing raises development MajMin from 58.6% to 60.8% with oracle timing and 53.7% to 55.7% end-to-end, but the holdout remains only 49.0% and 39.9%. The review flow makes smoothing reversible one suggestion at a time, but user-facing accuracy is not validated |
