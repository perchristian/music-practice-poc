...
The first mock-mode vertical slice, Phase 1 processed-song library, Phase 1B unified song workspace UX, Phase 2 real-mode FFmpeg extraction spike, Phase 2G piano-focused real separation spike, Phase 2G-QA Demucs bakeoff, Phase 2H real harmonic-analysis spike, Phase 3 musical grid calibration/editable compact chord chart UI, Phase 4A single grid-snapped loop pass, Phase 5A runnable Flat Sections prototype, Phase 5B.1-5B.3 section-range hygiene/rendering/creation pass, and Phase 5C.1 practice-state save reliability are implemented and functionally verified by automated backend and browser tests. A developer can start the local web POC, switch active pipeline mode from the GUI, queue one or more media files for mock processing, keep active jobs visible in one primary song list, reopen completed processed songs from that same list, select songs without navigating through separate Recent and All songs pages, rename/delete the selected song from the header's more menu, persist practice state with visible saving/saved/failure feedback, mute/unmute stems, solo stems, adjust stem volume, use playback controls, see beat/bar markers, turn on a grid-aligned metronome/click from the mixer row, manually halve/double or type-correct the displayed tempo, set bar 1 start including negative values when the recording begins just after the first downbeat, switch time signature with 4/4, 3/4, 5/4, 6/8, 7/8, and 12/8 options, change key with natural/sharp/flat spellings, see roman numerals update to the selected key, use bar-based loop controls when a grid exists, use a 1-bar loop count-in that clicks before loop audio starts and before every repeated loop pass, see the active loop in Harmony as a continuous row-chunk range, adjust loop start/end by dragging Harmony handles across rows, edit the working chord chart by editing chord labels, dragging chords to new beat positions, adding chords from `+` buttons in empty cells, deleting chords with a small corner `x`, and label song sections such as intro/verse/chorus as flat bar ranges. Pending practice edits are stored as job-bound snapshots, serialized, and flushed before song switches, so the debounce cannot accidentally save one song's settings to another. Grid corrections persist separately from analyzer metadata in `practiceState.gridOverrides` as `bpm`, `beatsPerBar`, `beatUnit`, and `downbeatOffsetSeconds`; key correction persists as `practiceState.keyOverride`; user chord charts persist as grid-first `practiceState.chordChart` events with `bar`, `offsetDiv`, `durationDiv`, and `raw` chord text; flat section labels persist as `practiceState.sections`; and Harmony display settings persist as `practiceState.harmonyView` with `barsPerRow`, `chordDisplay`, and `sectionInfoVisible`. Section persistence is now normalized through shared frontend/backend section helpers that reject overlapping ranges while allowing adjacent ranges; existing sections can be edited with symbol, label, start, and end in a compact dialog; and new sections can be created by selecting bars directly in the Harmony grid instead of typing numeric start/end values. The corrected grid drives timeline markers, Harmony placement, current-chord highlighting, selected-song tempo text, bar-based loop conversion, and metronome clicks. Harmony now renders compact bar segments with small numeric bar markers, beat separators, optional 1/2/4/8 bars per row, name/roman display modes, active-loop overlays, a show/hide section-info toggle, subtle section `+` affordances on empty bars, and continuous section bands that show label/actions only once per section; chord cards can span following empty beats when their duration implies the chord still applies, and row layout keeps chord-card height consistent when sectioned and unsectioned bars share a row. The practice view is grouped into selected-song, Time, Harmony, and Stems/Mixer sections; learning status sits in the selected-song header; loop start/end/count-in only appear while Loop is enabled; dense full-song timelines suppress overly dense beat ticks and skip bar labels so numbers do not overlap; and selected video thumbnails are persisted and shown in the song row plus selected-song header when the browser can extract a frame. If local demo stems exist at `data/jobs/Bare piano.m4a` and `data/jobs/Uten piano.m4a`, mock mode uses them for piano/accompaniment; otherwise it falls back to generated drums/bass/guitar/piano WAV stems. In real mode, uploaded media is extracted to `source-audio.wav`, separated by Demucs `htdemucs_6s` into drums, bass, guitar, piano, vocals, and other stems by default, then analyzed for approximate beat-aware harmonic cues. During Demucs separation, the backend parses best-effort percentage output from Demucs stderr and maps it into the job's overall progress so the UI no longer remains stuck at 55% until completion. Completed jobs now expose `metadata.durationSeconds` from real WAV output when available; mock uploads can store browser-read original media duration and thumbnail data; and the song list prefers media duration over the fixed harmonic cue length.
