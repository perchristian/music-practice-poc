...
- Harmony renders analyzer chords as draft suggestions until `practiceState.chordChart` exists; once present, the user-edited grid-first chart becomes the displayed working chart while `job.result.metadata.chords` remains unchanged for provenance
...
- Completed Phase 3D grid-first chord chart model:
  ...
  - Changing BPM or Bar 1 start updates derived cue times while preserving each chord's musical `bar`, `offsetDiv`, and `durationDiv`.
...
Start Phase 3E: replace the plain Harmony cue list with a compact beat-aligned chord chart view/editor that uses the grid-first `practiceState.chordChart` model.
