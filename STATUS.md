...
Phase 4A reprioritized loop work on 2026-07-08: multiple saved loops, loop names, notes, and loop statuses are deferred. The next practice-loop priority is one good loop first. The UI now shows loop start/end as whole beat positions when a beat grid exists, while the backend continues to persist `practiceState.loopStart` and `practiceState.loopEnd` as seconds for compatibility. The count-in path is covered for nonzero `Bar 1 start`, and dense timeline rendering now drops nonessential beat ticks and skips overlapping bar numbers. Verification passed with `npm test`, focused Playwright loop/timeline tests, and full `npm run test:gui` with 19 tests. Manual listening on a real long song is still needed to confirm metronome behavior through repeated loop jumps.
