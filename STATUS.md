...
Shared timeline pinch regression fixed on 2026-07-23. The viewport now reserves horizontal touch gestures and two-finger pinch for the app while leaving vertical page scrolling to the browser. Active pinch pointers are captured before waveform/grid rerenders, single-finger horizontal touch panning remains available, touch coordinates no longer leak into later slider-zoom anchoring, and only the newest animation frame may apply a zoom anchor. Pinch works in ordinary playback with persisted `timelineView.zoom` and in `Edit timing` with its separate ephemeral zoom; leaving edit mode preserves the ordinary value. Focused Playwright coverage synthesizes the two-pointer gesture in both modes. `npm test` passed with 54 tests, 53 passing and the optional unavailable local calibration skipped; all 29 Playwright tests passed. No test-created jobs remained, and all eight existing non-test library entries were preserved. No agents were delegated, and no model switch or skill was used.
