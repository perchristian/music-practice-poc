...
The next planned iteration is Phase 2B: real-mode upload and job contract, using the Phase 2A spike frame below.

Phase 2A spike frame:

- FFmpeg availability: unavailable locally on 2026-07-06. `command -v ffmpeg` returned no path and `ffmpeg -version` failed with `command not found`.
...
- Completed Phase 2A spike framing:
  - confirmed FFmpeg is unavailable locally
...
- Aligned Markdown phase references after the Phase 2 / Phase 3 roadmap order change:
  ...
  - `TASKS.md` now points the next task at Phase 2B instead of the already-completed Phase 2A.
