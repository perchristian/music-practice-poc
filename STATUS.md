...
Phase 0, Phase 1, Phase 1B, Phase 2A, and Phase 2B are complete for automated verification. Subjective audio usefulness still needs a human listening pass before external user testing.

The next planned iteration is Phase 2C: FFmpeg audio extraction worker.
...
- Phase 2B real-mode upload contract: `PIPELINE_MODE=real` accepts multipart media through `POST /api/jobs`, stores the uploaded source as `source.<original extension>` inside the job directory, returns queued/processing/failed states through `GET /api/jobs/:id`, and persists API-visible failures in `job.json`. The expected Phase 2B failure is that FFmpeg extraction is not implemented until Phase 2C.
...
- `PIPELINE_MODE=real` behind the same pipeline boundary, currently with real upload storage and an intentional API-visible failure before FFmpeg extraction
...
- Aligned Markdown phase references after the Phase 2 / Phase 3 roadmap order change:
  - `ARCHITECTURE.md` now defers notes and multiple named practice loops to Phase 3.
  - `TASKS.md` was updated to point the next task at Phase 2B instead of the already-completed Phase 2A.
...
Start Phase 2C: FFmpeg audio extraction worker.
