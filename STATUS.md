...
- Local test media: `test-media/phase-2a-source.wav`, a 6-second mono 44.1 kHz WAV generated by `npm run generate:test-media`.
...
- Real-mode upload and extraction contract: `PIPELINE_MODE=real` accepts multipart media through `POST /api/jobs`, stores the uploaded source as `source.<original extension>` inside the job directory, runs FFmpeg extraction to `source-audio.wav`, returns a completed practice-compatible result when FFmpeg succeeds, and persists API-visible failures in `job.json` when FFmpeg is unavailable or extraction fails.
...
- `PIPELINE_MODE=real` behind the same pipeline boundary, currently with real upload storage and FFmpeg source-audio extraction
...
- Completed Phase 2C through Phase 2F:
  ...
  - the extracted source audio is exposed as a practice-compatible `Extracted source audio` stem
...
Start Phase 3: problem areas and practice notes, or run a narrow piano-focused real stem-separation spike if the next iteration should reduce real-pipeline quality risk first.
