...
- per-song practice state stored in local `job.json`, including grid overrides, key override, and user chord edits
...
- Harmony renders analyzer chords as draft suggestions until `practiceState.chordEdits` exists; once present, the user-edited chart becomes the displayed working chart while `job.result.metadata.chords` remains unchanged for provenance
...
- Completed Phase 3C editable chord chart:
  ...
  - User chord edits persist as `practiceState.chordEdits`.
