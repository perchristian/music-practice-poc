...
The implemented Phase 2H spike currently records `harmonySource: "real-audio-analysis-v1"`, `analysisSource`, `beatGrid`, `beatOffsetSeconds`, `downbeatOffsetSeconds`, `downbeatConfidence`, per-cue `bar`, `beat`, `confidence`, and analyzer limitations. It emits beat-aware chord cues and merges repeated labels within a bar, while preserving repeated labels across bar boundaries. These cues are immutable analyzer suggestions in `job.result.metadata.chords`; they are evidence/provenance, not the user's source of truth. On first edit the app seeds a separate grid-first `practiceState.chordChart`, and that user working chart becomes authoritative for presentation, playback alignment, bar-based loops, roman numerals, and practice notes.
...
Phase 3G is the next timing architecture step because one BPM plus one offset cannot follow rubato or human tempo drift. The planned model keeps `job.result.metadata.beatGrid` immutable, retains `practiceState.gridOverrides` as the constant-tempo seed/fallback, and adds optional user-owned `practiceState.tempoMap` anchors shaped as `{ bar, timeSeconds }`. A pure shared timing module will map musical positions to audio seconds and back by piecewise-linear interpolation between strictly ordered downbeat anchors. Working chords and sections remain grid-first; their musical positions do not change when a user moves a bar line.

The timing editor will reuse the existing bar lines over a zoomable source waveform rather than introduce a separate anchor lane. Normal playback remains non-editable. `Edit timing` exposes waveform scrubbing, pan, edit-only zoom, bar-line drag, precise nudge, and anchor removal. Real mode will generate a compact versioned peak-envelope asset from `source-audio.wav`; mock mode will provide deterministic representative peaks without adding heavy dependencies. Raw full-source PCM will not be decoded in the browser for the first pass.

All time-sensitive consumers must use the shared map: timeline markers, metronome scheduling, chord cue timing/current highlighting, bar loops, Harmony loop overlays, and count-in. With a tempo map, displayed BPM is the average local segment tempo. Editing that tempo keeps the segment's left anchor fixed and moves only its right anchor while later explicit anchors retain their audio times. Mid-song meter changes, per-beat warping, smooth tempo curves, and automatic transient snapping remain deferred until the simpler model is tested on real recordings.
