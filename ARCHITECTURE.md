...
Phase 3G extends timing because one BPM plus one offset cannot follow rubato or human tempo drift. The implemented model keeps `job.result.metadata.beatGrid` immutable, retains `practiceState.gridOverrides` as the constant-tempo seed/fallback, and adds optional user-owned `practiceState.tempoMap` anchors shaped as `{ bar, timeSeconds }`. The pure shared `public/tempo-map.js` module maps musical positions to audio seconds and back by piecewise-linear interpolation between strictly ordered downbeat anchors. Working chords and sections remain grid-first; their musical positions do not change when a user moves a bar line.

The timing editor reuses the existing bar lines over a zoomable source waveform rather than introducing a separate anchor lane. Normal playback remains non-editable. `Edit timing` exposes waveform scrubbing, horizontal scrolling, edit-only zoom, bar-line drag, precise nudge, and anchor removal/reset. Real mode generates a compact versioned 80-peaks-per-second min/max envelope from `source-audio.wav`; mock mode provides deterministic representative peaks without adding heavy dependencies. The waveform is a separate job result asset, so raw full-source PCM is not decoded in the browser.

All time-sensitive consumers use the shared map: timeline markers, metronome scheduling, chord cue timing/current highlighting, bar loops, Harmony loop overlays, and count-in. With a tempo map, displayed BPM is the average local segment tempo. Editing that tempo keeps the segment's left anchor fixed and moves only its right anchor while later explicit anchors retain their audio times. Meter and `/2`/`x2` reinterpretation are disabled while a map exists; resetting the map restores the constant-grid controls. Per-beat warping, smooth tempo curves, and automatic transient snapping remain deferred. The real-song checkpoint showed that sparse downbeat anchors are useful, while later benchmark evidence and product review promoted nonzero-start handling, threshold-aware automatic tempo spans, and mid-song meter changes into the next timing architecture iteration.

The planned Phase 3G.3 model evolves the user-owned tempo map into one versioned musical timing map. Its ordered bar events may carry an explicit downbeat `timeSeconds`, an effective `timeSignature`, or both. Timing anchors and meter changes remain separate calculation concepts but share one persisted event and one visual bar line: dragging the line changes its audio time and adjacent derived tempo, while selecting that same line exposes exact time, local tempo, and the meter that begins at the bar. A meter-only event uses the calculated bar time, and removing one correction aspect must preserve any other aspect on that event. Existing version-1 `{ bar, timeSeconds }` anchors need deterministic normalization so intentional real-song calibration data is not discarded.

Tempo remains derived rather than independently persisted. The shared mapping must accumulate musical distance across each effective meter instead of assuming `(rightBar - leftBar) * beatsPerBar`; beat-unit and compound-meter pulse semantics must be explicit. Chords and sections remain grid-first, while timeline markers, click/accent scheduling, chord windows, loops, count-in, and inverse seeking all consume the same meter-aware map.

Automatic timing should become an explicit analysis stage before harmony segmentation. It should fit stable tempo spans across multiple observations, tolerate ordinary human beat variation, and create a new candidate boundary only when relative phase or tempo deviation crosses a calibrated threshold for a persistent window. Meter changes are candidates only at plausible downbeats and are attached to concrete bars. Analyzer timing provenance remains immutable and separate from accepted user timing corrections; user corrections may be compared as evaluation evidence but must not be silently learned from or overwritten.
