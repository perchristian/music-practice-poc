...
This is the append-only historical record split out of `docs/planning/STATUS.md` on 2026-07-25.
Nothing here was rewritten; the sections below are the original text moved
verbatim so that `docs/planning/STATUS.md` can stay a current-state document.

Use this file for evidence and provenance. Use `docs/planning/STATUS.md` for what is true now,
`CHANGELOG.md` for the curated outcome history, `docs/planning/TASKS.md` for the plan, and
`docs/planning/DECISIONS.md` for irreversible choices.

## Prior Current-Status Summary

The single-paragraph current-state summary that headed `docs/planning/STATUS.md` until
2026-07-25, kept verbatim. It was replaced by a structured summary; no fact was
removed.

The first mock-mode vertical slice, Phase 1 processed-song library, Phase 1B unified song workspace UX, Phase 2 real-mode FFmpeg extraction spike, Phase 2G piano-focused real separation spike, Phase 2G-QA Demucs bakeoff, Phase 2H real harmonic-analysis spike, Phase 3 musical grid calibration/editable compact chord chart UI including Phase 3G.3 unified timing and Phase 3G.2 playback timeline zoom/Follow, Phase 4A single grid-snapped loop pass, Phase 5A runnable Flat Sections prototype, Phase 5B.1-5B.3 section-range hygiene/rendering/creation pass, and Phase 5C.1 practice-state save reliability are implemented and functionally verified by automated backend and browser tests. A developer can start the local web POC, switch active pipeline mode from the GUI, queue one or more media files for mock processing, keep active jobs visible in one primary song list, reopen completed processed songs from that same list, rename/delete the selected song, persist practice state with saving/saved/failure feedback, mute/solo/mix stems, and use playback controls. The ordinary timeline exposes persisted Zoom, Fit, and optional Follow; Follow defaults off and yields to manual panning. `Edit timing` exposes a separately zoomable waveform where one selected bar line can own an explicit downbeat time, an effective time signature, or both; those aspects can be removed independently. The same meter-aware map drives contextual BPM, timeline/bar enumeration, click/accent scheduling, chord windows/highlighting, loops, Harmony, count-in, and inverse seeking. Bar 1 supports negative time, count-in works with or without a loop, and 6/8 plus 12/8 use dotted-quarter pulse grouping. Constant corrections persist in `practiceState.gridOverrides`; sparse variable timing and meter changes persist in version-2 `practiceState.timingMap`; version-1 tempo anchors migrate deterministically. Timeline viewing persists separately in `practiceState.timelineView`. Key, chord chart, sections, and Harmony view remain separate user-owned state. In real mode, uploaded media is extracted to `source-audio.wav`, separated by Demucs, passed through threshold-aware timing before approximate harmony analysis, and reduced to a compact waveform envelope; mock mode remains dependency-light.
...
owner had closed it with a `CHANGES` verdict on 2026-07-24; the amendment is now
recorded in `docs/planning/DECISIONS.md` Decision 33, in an `amendments` block in
`benchmarks/chord-reliability-contract-v1.json`, in a new amendment section of
`docs/research/chord-reliability-cr0-contract.md`, and as an Outcome in the approval
packet. RWC-P becomes the primary benchmark, the two untouched target holdouts
and Logic fixtures leave the critical path, and a manual domain check on a few
representative iOS screen recordings becomes the final step. Because the RWC
holdout became the primary gate without an absolute accuracy bar, a proposed
`thresholds.rwcPrimaryGate` block was added with
`status: proposed-awaiting-approval`; it is the one deferred product-owner input.
Second, `docs/planning/DECISIONS.md` contained two different decisions numbered 18; the
harmonic-analysis-spike entry was renumbered to 32 with its text and date
unchanged. Third, `docs/planning/STATUS.md` was reduced from about 120 KB to about 10 KB by
moving the dated history, completed-work list, verification log, and older skill
use verbatim into this archive. Fourth, `npm run test:gui` now runs its server
against a temporary `DATA_DIR` created per run, so browser tests can no longer
write into the local song library; `AGENTS.md` records that the automated suites
no longer need cleanup. Verification: `npm test` passed with 55 passing and one
optional local calibration skipped; `npm run test:gui` passed all 29 tests in
1.9 minutes and left the nine existing `data/jobs` directories byte-identical;
`npm run verify:chord-contract` passed against the amended contract and reported
no application-job mutation; syntax and `git diff --check` passed. An earlier run
of the browser suite failed one timeline test and took 11.6 minutes, which was
reproduced as CPU contention from concurrent probing rather than the isolation
change: a direct measurement showed mock job completion at 3.69 s with both an
empty temporary data directory and a copy of the real one. No analyzer or runtime
behavior changed. No agents were delegated and no model switch was used.

Real-mode upload limit update on 2026-07-23: the hard multipart media cap increased from 150 MB to 650 MB so a five-minute recording at the observed bitrate of a 336 MB, 2:49 screen recording fits with headroom. `node --check server.js`, `node --check tests/real-mode-upload.test.js`, `git diff --check`, and `npm test` passed; the suite reported 53 passing tests and 1 skipped local calibration test, including the 650 MB oversized-request contract. Tests used isolated temporary data directories and left no test-created library jobs. The current multipart parser still buffers uploads in memory, so near-limit memory and disk use remain an explicit risk. No agents were delegated, and no model switch or skill was used.

Chord-analysis strategy review added on 2026-07-23. `docs/engineering/CHORD_ANALYSIS_STRATEGY.md` traces the current real-mode path from timing and weighted full-mix/stem chroma through template scoring, isolated-beat smoothing, key/roman derivation, immutable analyzer evidence, and the separate user-owned working chart. It explicitly distinguishes the fixed mock progression, reports the locked RWC-P checkpoint, documents observed screen-recording uncertainty, and provides review questions for the next accuracy decision. This documentation-only iteration changed no runtime behavior and created no songs or jobs. No agents were delegated, and no model switch or skill was used.

Chord Reliability Validation Gate committed on 2026-07-23. Product review concluded that reliable chord analysis is core alongside stem separation and must be proven good enough before unrelated feature work resumes. `docs/research/chord-reliability-validation-plan.md` records the MIR research, current failure analysis, accompaniment-first evidence model, reliability-gated bass fallback for no-bass mixes, ornament-resistant comp evidence, repetition-aware pooling, locked evaluation contract, provisional usability thresholds, seven milestones, and terminal GO/ADJUST/COMPARE/STOP decisions. `docs/planning/TASKS.md` now orders CR0-CR6 as the committed sequence; prior work packages are deferred. `docs/planning/RISKS.md` elevates chord reliability to a high-likelihood/high-impact core blocker, `docs/planning/DECISIONS.md` records the prioritization, and `docs/engineering/CHORD_ANALYSIS_STRATEGY.md` links the forward plan. This planning iteration changed no runtime behavior and created no songs or jobs. No agents were delegated, and no model switch or skill was used.
...
CR0 Codex preparation completed on 2026-07-24. `benchmarks/chord-reliability-contract-v1.json` checksum-locks a fresh stratified 8-development/4-holdout RWC-P split that excludes all 12 consumed Phase 2J tracks, five local target candidate identities, exact reference/corrected and end-to-end metrics, artifact paths, accuracy/usability/regression/performance thresholds, and holdout access rules. No current screen recording qualifies as untouched full-length target holdout: four proposed files have already influenced the POC and the remaining alternate is only 17 seconds with unconfirmed prior-review status. The contract therefore reserves two new full-length untouched target holdout slots covering full-band/bass/repeat variation and no-bass/melody/ornaments. `docs/research/chord-reliability-cr0-contract.md` defines analyzer-blind timing, chord, and section transcription followed by verification from a different musician. The benchmark CLI now requires `--allow-holdout` to produce locked results, while `npm run verify:chord-contract` verifies manifest/media/archive checksums, annotation commit, selected members, official reference intervals, target media properties, both dry-run timing modes, and no application-job mutation. Verification passed with 55 automated tests and one optional calibration skipped; the CR0 dry run left all nine pre-existing job directories and their `job.json` content unchanged. No analyzer behavior or public output changed. Product-owner approval packet [#3](https://github.com/perchristian/piano-practice-poc/issues/3) is the only ready human action. No agents were delegated and no model switch was used; the `github:github` skill guided issue inspection and the just-in-time handoff.

Issue #3 fixture clarification recorded on 2026-07-24. Per confirmed that original Logic Pro arrangements can be authored from an explicit chord progression with drums, bass, keys, optional vocals, and simple-to-complex Session Player performances, then exported as the full mix, source stems, and MIDI. This is a practical route to controlled, locally usable answer-key fixtures and, if predeclared before analyzer access, untouched screen-recording candidates. Existing generated sine-wave fixtures already cover deterministic timing and basic harmony, RWC-P supplies annotated realistic-pop regression evidence, and the existing `MakeYouFeelMyLovePart2` Logic files are outputs from Logic Pro Stem Splitter rather than original source multitracks. The recommended addition is one reusable Logic development/calibration pack; the two CR0 holdouts should remain untouched and should still cover the locked full-band and no-bass target scenarios. MIDI and the Logic chord track can support reference preparation, but the audible bounce remains authoritative and the locked independent second-musician review still applies. This was a read-only repository and issue assessment: no analyzer, tests, songs, or jobs ran or changed. No agents were delegated and no model switch was used. The `github:github` skill was used to inspect issue #3 and its relationship to the local benchmark assets; the result is reproducible without the skill from issue #3 plus `docs/research/chord-reliability-cr0-contract.md`, `benchmarks/chord-reliability-target-v1.json`, `scripts/generate-phase2a-test-media.js`, and `scripts/build-stem-bakeoff.js`.
...
- Real-mode dependency setup: `requirements-real.txt` contains optional heavy dependencies; the default invocation is `.venv-real/bin/python -m demucs`, with `DEMUCS_PYTHON` and direct-entrypoint `DEMUCS_PATH` overrides. Local verification used `demucs==4.0.1`, `torchcodec==0.14.0`, and `TORCH_HOME=.cache/torch`.
- Stem bakeoff fixture: `test-media/MakeYouFeelMyLovePart2.mov` with manual Logic Pro stems under `test-media/stems from logic/`.
- Stem bakeoff command: `npm run bakeoff:stems` imports the Logic baseline and creates the FFmpeg spectral comparison job; `TORCH_HOME=.cache/torch npm run bakeoff:stems -- --demucs` also creates the Demucs `htdemucs_6s` comparison job.
...
Phase 3G.1 product-feedback follow-up implemented on 2026-07-17. Downbeats can now be locked at their calculated position with one click before an adjacent line is adjusted, which supports short ritardando/accelerando spans without first displacing the boundary. A corrections picker with previous/next navigation includes Bar 1 and exposes exact-time editing plus icon-only nudges. Zoom anchors to the playhead, pointer, or two-finger pinch center; mapped BPM refreshes while scrubbing and while playback crosses a segment; and a red capped playhead is visually distinct from teal downbeats. `Start at Bar 1` is persisted independently, and count-in is now a transport setting usable without a loop while still repeating on loop passes; it defaults off for new jobs. Automated verification covers API persistence, click-to-lock correction lists, local-BPM transitions, Bar 1 playback start, independent/repeated count-in, and existing timing consumers. `npm test` passed with 27 passing/1 skipped tests, all 26 Playwright tests passed including 390 px overflow coverage, and a 1280 px rendered editor was visually inspected. The one demo job created for visual inspection was removed; existing user/calibration jobs were left intact. No agents were delegated and no model switch or skill was used.

The Phase 3G.1 real-song checkpoint passed on 2026-07-17. The user aligned `TeAmo.mov` with 16 timing anchors and reported that the corrected timing worked excellently. This validates sparse piecewise-linear bar anchors as sufficient for that recording and unblocks Phase 2J calibration; broader recordings and smoother intra-bar tempo behavior remain uncertain.

Phase 2J corrected-timing reanalysis implemented on 2026-07-17. A real song with a tempo map now exposes `Reanalyse chords` in Harmony. After an explicit replacement confirmation, the backend reads the existing `source-audio.wav`, creates chord-analysis windows from every corrected beat boundary, and stores the new key/chords under `metadata.correctedTimingAnalysis` while retaining the original `metadata.chords` as immutable provenance. The previous working chart is stored in `practiceState.chordChartBackup` before the new suggestions become active. Backend coverage verifies variable beat windows, original-analysis retention, backup persistence, and real FFmpeg-pipeline reanalysis; Playwright verifies the guarded action and visible result replacement. Verification passed with 28 passing/1 skipped Node tests and all 27 Playwright tests. The feature was then run on the calibrated 3:48 `TeAmo.mov`: it produced 279 corrected-timing cues, estimated A minor at 0.44 confidence, retained all 170 original analyzer cues, and backed up all 95 previous working-chart events. The density is now a deliberate Phase 2J review target rather than an inferred quality result. No agents were delegated and no model switch or skill was used.

Corrected-timing key handling was tightened on 2026-07-17 after review showed that `TeAmo.mov` had a user-selected C-major key but reanalysis still stored an internal A-minor estimate. Reanalysis now treats `practiceState.keyOverride` as authoritative for the derived analysis key and roman numerals, records `analysis.keySource: user-override`, and estimates key only when no user choice exists. Chord-name scoring remains chromatic rather than restricted to the key so borrowed and secondary harmony can still appear. `TeAmo.mov` was reanalysed with C major, producing the same 279 chord-name cues with corrected C-major roman numerals; the 95-event working-chart backup remains intact. Verification passed with 28 passing/1 skipped Node tests and all 27 Playwright tests. No agents were delegated and no model switch or skill was used.

Changelog process established on 2026-07-17. A new root `CHANGELOG.md` backfills notable app and developer-facing changes from the Git history and the verified outcomes in `docs/planning/TASKS.md`/`docs/planning/STATUS.md`, grouped into dated POC milestones for 2026-07-05, 2026-07-06, 2026-07-07, 2026-07-08, and 2026-07-17. `AGENTS.md` now requires completed implementation iterations to add concise `Added`, `Changed`, `Fixed`, or `Removed` entries under `Unreleased` in the same focused commit; planning-only and routine internal changes are excluded unless they materially affect development. `README.md` links the changelog and clarifies the roles of the main project documents. Documentation-only change; no runtime tests or jobs were needed. No agents were delegated and no model switch or skill was used.
...
Phase 2J analysis reset/undo implemented on 2026-07-22. `Back to analysis` now confirms before replacing any non-null working chart, stores the exact normalized chart with the active original or corrected-timing analysis identity, and returns to immutable conservative suggestions. `Undo reset` is available once only while no newer working chart exists and the same analysis remains active; successful restore consumes the backup. Corrected-timing reanalysis now assigns a fresh identity, makes a displaced chart immediately undoable against that new analysis, and clears stale backups when no chart was displaced. Backend coverage exercises ordinary mock reset/undo and corrected-timing restore; browser coverage performs reanalysis, hidden-candidate recovery, confirmed reset, and exact undo in one journey. Verification passed with 41 Node tests and one optional local calibration skipped, all 28 Playwright tests, syntax checks, and clean diff checks; the isolated test environments created no library jobs. This completes the bounded Phase 2J checkpoint; absolute accuracy remains a documented risk. No agents were delegated, and no model switch or skill was used.
...
Phase 3G.3 implemented for automated verification on 2026-07-22. `practiceState.timingMap` version 2 stores ordered bar events with optional `timeSeconds` and `timeSignature`; legacy version-1 anchors normalize with the active initial meter. One selected waveform bar line exposes exact time, local derived tempo, and meter-from-this-bar, with independent removal and a full reset. The pure map accumulates written beats and musical pulses through meter changes; 6/8 and 12/8 use dotted-quarter pulses, while 7/8 keeps eighth-note pulses because grouping is unknown. Timeline, click/accent, chord windows/chart layout, loops, count-in, Harmony, and inverse seeking share the map. Real analysis now fits sparse timing spans before chord segmentation after an 18% phase or 8% local-tempo deviation persists for four beats; meter candidates require two bars and remain immutable analyzer provenance. `npm run benchmark:timing` passes seven generated fixtures with no candidates for ordinary jitter, exact tempo/meter boundary bars, and at most 60 ms bar error. `npm test` passed with 52 tests and one optional local calibration skipped; all 28 Playwright tests passed after the intentional persistence-contract update. Test-created jobs were removed; the four pre-existing local job directories remain. Manual review on two real screen recordings is still required. No agents were delegated, and no model switch or skill was used.

The first partial Phase 3G.3 human checkpoint was recorded on 2026-07-22 using `ShapeOfMyHeart manually adjusted`. Manual correction of Bar 1 and several later downbeats is valid checkpoint setup because the gate tests whether the corrected user map becomes authoritative, not whether import analysis is initially perfect. Human listening passed metronome alignment, chord-display transition timing, repeated loop count-in, and persistence after reload. Chord names remained inaccurate after corrected-timing reanalysis; this confirms a harmonic-analysis limitation without invalidating the timing pass. Loop testing exposed that Play could start from a paused playhead outside the enabled range, causing an initial count-in there and a second count-in when transport crossed the loop end. Playback now enters at loop start whenever Play is invoked before or at/after a valid loop, while paused inspection outside the range remains possible. The focused regression reproduced the failure before the fix and passed afterward; `npm test` passed with 52 tests and one optional local calibration skipped, and all 28 Playwright tests passed. No test-created jobs remained. Human confirmation of the loop-entry fix passed; the required late-start and meaningful tempo/meter-change recordings are still outstanding. No agents were delegated, and no model switch or skill was used.
...
Shared timeline pinch follow-up attempted on 2026-07-23 but did not pass human Mac trackpad review. The pointer-based implementation reserves horizontal touch gestures for the viewport, captures synthetic pinch pointers across rerenders, keeps timing-edit zoom separate, and passed focused synthetic Playwright coverage plus the full automated suites. Human review then found the real behavior remains intermittent: trackpad pinch may begin only after moving the slider to roughly 30% and can stop again after returning to minimum zoom. The automated test models touchscreen-style `PointerEvent` input, while a Mac trackpad normally reports two-finger pan and pinch through wheel gesture events, so the test did not validate the actual input path. `docs/engineering/TIMELINE_INTERACTION_CONTRACT.md` separates the pending Mac trackpad, mouse, touchscreen, and keyboard contract. The reliable current path is the Zoom slider plus `Fit`; no additional implementation was made during this documentation review. No agents were delegated, and no model switch or skill was used.

Timeline interaction product review completed on 2026-07-23. `docs/engineering/TIMELINE_INTERACTION_CONTRACT.md` defines one input-mechanism-based contract for trackpad, mouse, touchscreen, and keyboard navigation, including click/drag arbitration, browser-managed vertical touch scrolling, zoom anchors, an accessible horizontal scrollbar, timing-marker precedence, and edit-viewport restoration. The agreed Follow policy now affects viewport position only while playback is running and never changes playback time; this intentionally supersedes Phase 3G.2's paused-seek behavior. `docs/planning/TASKS.md` adds Phase 3G.2A as the next corrective implementation slice with browser-faithful automation and a required real Mac trackpad gate, after which work returns to Phase 5D.2. This was a documentation-only planning iteration: no runtime code, tests, songs, or jobs changed. No agents were delegated, and no model switch or skill was used.

Documentation organization follow-up completed on 2026-07-23. The product-reviewed timeline interaction contract moved out of `docs/product/UX_FLOOR_PLANS.md` into the focused `docs/engineering/TIMELINE_INTERACTION_CONTRACT.md`; task and demo references now point to the new file, while `docs/product/UX_FLOOR_PLANS.md` remains scoped to the unified song workspace layouts and links to the contract. No runtime behavior, tests, songs, or jobs changed. No agents were delegated, and no model switch or skill was used.

Project retrospective and AI-context review completed on 2026-07-23. `docs/planning/RETROSPECTIVES.md` now provides an append-only home for dated reports. `docs/engineering/AI_TOKEN_OPTIMIZATION.md` was revised from a static, estimate-led proposal into a measurement-first plan covering current-context condensation, stable codebase orientation, symbol-based task contracts, active-seam module extraction, boundary-focused JSDoc, and before/after context recovery. `docs/planning/TASKS.md` now pauses implementation for product-owner backlog grooming and sprint planning and groups remaining work into provisional technically sequenced work packages. This planning order intentionally favors dependency, technical risk, and reversibility for grooming and must be rebalanced against learning value before a sprint is selected. No runtime code, tests, songs, or jobs changed. No agents were delegated, and no model switch or skill was used.

## Completed Work

Moved verbatim from `docs/planning/STATUS.md`.


- Created `docs/product/VISION.md`.
- Created `docs/engineering/ARCHITECTURE.md`.
- Created `docs/planning/TASKS.md`.
- Created `docs/planning/RISKS.md`.
- Created `docs/planning/DECISIONS.md`.
- Created `docs/planning/STATUS.md`.
- Created `docs/engineering/DEMO.md`.
...
- Added a phased forward roadmap in `docs/planning/TASKS.md`:
...
- Documented native iOS transition criteria in `docs/engineering/ARCHITECTURE.md`.
...
- Documented the Phase 1 storage decision in `docs/planning/DECISIONS.md`.
...
- Refined `docs/product/VISION.md` validation criteria into an OKR-style objective with key results for learning speed, perceived value, motivation, empowerment, transfer learning, error tolerance, return intent, and adoption threshold.
- Added `docs/product/UX_FLOOR_PLANS.md` with three floor plans for a Voice Memos-inspired unified song workspace.
- Documented the unified song workspace direction in `docs/engineering/ARCHITECTURE.md`, `docs/planning/TASKS.md`, and `docs/planning/DECISIONS.md`.
...
- Aligned Markdown phase references after the Phase 2 / Phase 3 roadmap order change:
  ...
  - `docs/planning/TASKS.md` was later reprioritized so Phase 3 is grid calibration/editable chords and Phase 4 is bar-based loops/practice notes.
...
Moved verbatim from `docs/planning/STATUS.md`. Dated verification evidence for completed work.
...
- Markdown consistency audit on 2026-07-06: searched project Markdown for Phase 2 / Phase 3, first real-pipeline, problem areas, and practice-notes references; corrected stale references in `docs/engineering/ARCHITECTURE.md` and `docs/planning/TASKS.md`. Documentation-only change; no code tests run.
...
- Documentation-only validation criteria update in `docs/product/VISION.md`; no code verification run.
...
- Manual subjective listening could not be completed by Codex; a human should still listen through `docs/engineering/DEMO.md` before external user testing.
...
- Updated `docs/engineering/DEMO.md` to clarify that Demucs real mode should show `Backend ready: real · demucs-htdemucs_6s`.
