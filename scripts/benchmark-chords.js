...
Options:
  ...
  --manifest PATH      locked pilot manifest
...
function parseArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    index += 1;
    result[key] = pathKeys.has(key) ? resolve(value) : value;
  }
...
async function createManifest(options) {
  ...
  const selected = selectStratifiedPilot(tracks, 12);
  const holdoutIndexes = new Set([1, 4, 7, 10]);
  const annotationCommit = await readGitHead(options.annotations);
  const manifest = {
    version: 1,
    benchmark: "RWC Popular Music Database v2 chord pilot",
    dataset: {
    ...
    selection: {
      method: "Weighted standardized chord-density, vocabulary, entropy, tempo, duration, and instrumentation features; three equal complexity strata with four deterministic quantile samples each.",
      policy: "Eight development tracks and four untouched holdout tracks. Do not tune against holdout results."
    },
  ...
  console.log(`Wrote locked 8/4 pilot manifest to ${options.manifest}`);
