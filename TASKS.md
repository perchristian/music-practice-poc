...
Status: Complete on 2026-07-22. Normal playback exposes persisted per-song Zoom, Fit, and optional Follow on the same timeline viewport. Follow targets 37.5% of the viewport, clamps at the song boundaries, updates through paused seeks, playback-speed changes, and loop relocation, and turns off on direct pan input. Timing edit keeps its own ephemeral zoom and suppresses Follow without changing the saved playback preference. Focused browser coverage verifies anchoring, reload, loop entry, manual-pan cancellation, and no page overflow at 1180, 820, and 390 px. A 2026-07-23 regression fix restored pointer-captured two-finger pinch in both modes, retained one-finger horizontal touch panning, and prevents stale animation frames from changing a newer zoom anchor. Follow remains off by default until a long-song manual review provides evidence to change that default.
