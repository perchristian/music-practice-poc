...
Status: Implemented and complete for automated verification on 2026-07-22. Version-1 tempo anchors migrate to version-2 timing events; the shared mapping, editor, click, chord windows, loops, count-in, Harmony, and inverse seeking are meter-aware; and timing/meter correction aspects share one bar line but remain independently removable. The automatic timing stage records sparse candidates/provenance before chord segmentation. Seven generated known-answer fixtures pass the locked timing gate, including jitter rejection, tempo changes, accelerando/ritardando, and 4/4-to-3/4. Human review now covers two real recordings: `ShapeOfMyHeart manually adjusted` confirms that the metronome follows manually aligned bars through changing tempo, and a second late-start recording passed click, loop, repeated count-in, and reload. Loop review also established and accepted the rule that an enabled valid loop always relocates to its start before direct playback or count-in. The late-start review found two chord-highlight integration defects: corrected-timing cues retained reanalysis-time waveform seconds after Bar 1 moved, then rounded analyzer seconds could place a cue declared for the next bar onto the previous bar's final beat. Corrected-analysis cues now remap from their immutable timing snapshot, explicit bar/beat is authoritative over rounded seconds, and near-integer beat ends snap to their intended boundary. Focused unit/browser and full automated coverage pass. Human confirmation at an exact corrected downbeat remains the only checkpoint gate. Chord labels on `ShapeOfMyHeart manually adjusted` remained inaccurate after corrected-timing reanalysis, which is harmonic-quality evidence rather than a failure of the shared timing map.
