...
20. Increase `Zoom` with the slider over the waveform and confirm zoom stays centered on the playhead or pointer. Horizontally scroll to a known downbeat. Trackpad pinch is not part of the reliable demo path until the pending gesture contract passes human review.
...
22. With a bar line selected, change `Meter from this bar` and confirm the same line marks the meter boundary. `Remove time` and `Remove meter` clear those aspects independently; `Reset all` returns to analyzer/base timing. Confirm the line is visually distinct from the red playhead, contextual BPM and click/chord timing update immediately, then reload and confirm the events return.
...
24. If Harmony shows `Review hidden chords`, open it and audition one of the isolated one-beat changes that conservative smoothing removed. Select `Add` only when it sounds useful; confirm the surrounding working chord remains on both sides and the edit reaches `Saved`. Select `Back to analysis`, confirm the reset, and verify the conservative chart returns; then select `Undo reset` and confirm the exact working chart returns once.
25. Adjust click volume from the mixer row; downbeat accent is always on for now.
26. Set count-in to `1 bar` without enabling Loop and confirm it prepares ordinary playback. Optionally enable `Start at Bar 1` to skip incomplete pre-roll. Then enable Loop and change its start/end; `1` means all of Bar 1. While paused, seek before, inside, and beyond the loop; each time, press Play and confirm the playhead first moves to loop start, one count-in runs there, and playback begins at that boundary. Confirm count-in repeats before each subsequent loop pass, including after timing corrections.
27. Change playback speed, stem volume, grid click settings, tempo correction, time signature, key, and learning status. The global time signature and `/2`/`x2` controls are intentionally disabled while mapped timing exists; select a bar line to change the meter from that bar.
...
10. Enter `Edit timing`, select Bar 1 in Corrections, and adjust its exact time or arrow nudges while listening to the click. Negative values are allowed when recording starts just after the downbeat. Change `Meter from this bar` if accents are wrong.
...
- Variable timing uses constant tempo inside each span between sparse timed bar events. Mid-song meter changes are supported, but expressive accelerando/ritardando may need several events; smooth curves and per-beat warping are not implemented. Automatic timing candidates passed generated fixtures but still require listening validation on varied real screen recordings.
